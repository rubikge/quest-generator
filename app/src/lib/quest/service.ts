...
export type WeaveFn = (args: {
  theme: Theme;
  level: Level;
  // The three coding tasks (canonical English) the model localizes + frames; deployment framing is
  // generated by the flow. The model never receives or returns grading-authoritative data.
  tasks: Array<{
    taskId: string;
    sourceUrl: string;
    title: string;
...
export async function generateQuest(
  ...
  const session: Session = {
    ...
    missionInputs: {},
    // Persist the auto-detected display language (English fallback) for this session (US3/FR-014).
    ...(narrative.detectedLanguage ? { detectedLanguage: narrative.detectedLanguage } : {}),
    updatedAt: deps.now(),
  };
...
export type VerifyDeploymentResult =
  ...
  | { ok: false; code: 'MISSING_LINKS'; message: string; missingTaskIds: string[]; missingLinks: string[] };
...
export async function verifyDeployment(
  ...
  const coding = session.quest.missions.filter((m) => m.kind === 'coding' && m.taskId && m.sourceUrl);
  const taskIds = coding.map((m) => m.taskId as string);
  // US4: require the README to also link each task's original ACMP page (FR-015/FR-016).
  const missions = coding.map((m) => ({ taskId: m.taskId as string, sourceUrl: m.sourceUrl as string }));
  const result = await verifyRepo({ repoUrl: input.repoUrl, taskIds, missions, fetchImpl: deps.fetchImpl });

  if (!result.ok) {
    if (result.code === 'MISSING_LINKS') {
      return { ok: false, code: 'MISSING_LINKS', message: result.message, missingTaskIds: result.missingTaskIds, missingLinks: result.missingLinks };
    }
